...
## ---------------------------------------------------------------------
## ASSIGN_BASIS -- which rule names an institution's category
##
##   "counts"  the ranking cut above: the category lines are drawn through
##             the size-ordered list so that the tallies equal the
##             probability-sum targets. Counts are then the model's
##             expected counts, including the backtest correction to the
##             $10B category, but a row's label can differ from the band
##             its median falls in -- which is what field staff noticed.
##
##   "median"  the label IS the band the median forecast falls in. Every
##             row reads consistently and the counts are a plain tally of
##             the list. The cost: the median is the uncorrected central
##             estimate, so the $10B count reverts to roughly the
##             uncorrected figure and the [23.6b] correction no longer
##             reaches the published counts. [24.2b] prints both so the
##             difference is visible before anything is published.
## ---------------------------------------------------------------------
ASSIGN_BASIS <- cfg_get("ASSIGN_BASIS", "median")

## BREAKS is c(0, 10e6, 50e6, 100e6, 500e6, 1e9, 10e9): the same edges the
## probabilities are read at, so label and number cannot disagree.
cat_of_median <- function(med) {
  k <- findInterval(med, BREAKS[-1], left.open = FALSE) + 1L
  pmin(pmax(k, 1L), N_CAT)
}
stopifnot(identical(cat_of_median(inst$assets_now), as.integer(inst$cat_k)))

for (h in H_SET) {
  med <- inst[[paste0("assets_med_h", h)]]
  k   <- if (ASSIGN_BASIS == "median") cat_of_median(med) else
           assign_cut(med, TARGET[[as.character(h)]]$target,
                      inst$assets_now, inst$join_number)
  k_cut <- assign_cut(med, TARGET[[as.character(h)]]$target,
                      inst$assets_now, inst$join_number)
  inst[[paste0("k_cut_h", h)]] <- k_cut
...
## ---------------------------------------------------------------------
## [24.2b] What the two rules give. Read this before publishing.
## ---------------------------------------------------------------------
cmp <- lapply(H_SET, function(h) {
  data.frame(cat = CAT_LABELS,
             by_median = as.integer(table(factor(inst[[paste0("k_assign_h", h)]],
                                                 levels = 1:N_CAT))),
             by_counts = as.integer(table(factor(inst[[paste0("k_cut_h", h)]],
                                                 levels = 1:N_CAT))),
             soft = round(TARGET[[as.character(h)]]$soft, 1))
})
names(cmp) <- as.character(H_SET)
for (h in H_SET) {
  cat("\nh =", h, " (ASSIGN_BASIS =", ASSIGN_BASIS, ")\n")
  print(cmp[[as.character(h)]] %>% mutate(diff = by_median - by_counts) %>%
          as.data.frame(), row.names = FALSE)
}
cat("\nby_counts reproduces the probability sums (soft), including the\n",
    "$10B correction. by_median is a tally of where each median lands and\n",
    "does not. Where they differ, the difference is the correction and the\n",
...
for (h in H_SET) {
  ...
  if (ASSIGN_BASIS == "median") {
    cat(sprintf("h=%2d  assigned by median: %d institutions (target was %d; ",
                h, sum(got), sum(tgt)))
    cat(sprintf("largest category %d vs %d)\n", got[N_CAT], tgt[N_CAT]))
    stopifnot(sum(got) == N_COHORT)
  } else {
...
## data supports.
## ---------------------------------------------------------------------
DOWN_N   <- cfg_get("DOWN_N", 30)        # how many to name
DOWN_MIN <- cfg_get("DOWN_MIN", 0.02)      # floor: below this, do not name at all
